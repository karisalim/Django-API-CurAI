...
class JWTAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        encoded_jwt = auth[1]
        user = None  # تعريف المتغير مسبقًا
        ...
        try:
            ...
            if not user.is_active:
                raise AuthenticationFailed(_('User account is not activated.'))

        except jwt.ExpiredSignatureError:
            raise AuthenticationFailed(_('Expired JWT.')) from None
        except jwt.InvalidTokenError:
            raise AuthenticationFailed(_('Invalid JWT.')) from None
        except user_class.DoesNotExist:
            raise AuthenticationFailed(_('User not found.')) from None
